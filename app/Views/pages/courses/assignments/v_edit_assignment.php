...
<div class="bg-gray-200 rounded-md px-4">
    <div class="breadcrumbs mb-6">
        <ul>
            ...
            <li class="font-semibold">Edit Assignment</li>
        </ul>
    </div>
</div>
    ...
    <form id="assignmentForm" action="<?= url_to('update_assignment', $assignment->id) ?>" method="post"
        ...
        <fieldset class="fieldset">
            ...
            <textarea name="description" class="textarea textarea-bordered w-full" placeholder="Description"
                required><?= esc($assignment->description) ?></textarea>
        </fieldset>

        <fieldset class="fieldset">
            <legend class="fieldset-legend">Due Date</legend>
            <input type="datetime-local" name="due_date" class="input w-full"
                value="<?= date('Y-m-d\TH:i', strtotime($assignment->due_date)) ?>" required>
        </fieldset>

        <fieldset class="fieldset">
            <legend class="fieldset-legend">Replace File <span class="text-xs text-gray-500">(Leave empty if no
                    changes)</span></legend>
            <input type="file" name="file" id="fileInput" class="file-input w-full">
        </fieldset>

        <div class="mt-4">
            <?php if (!empty($assignment->file_url)) : ?>
            <iframe src="<?= url_to('file_assignment', $assignment->course_id, $assignment->file_url) ?>"
                id="file-preview" class="w-full border rounded-xl shadow-md" height="500">
            </iframe>
            <?php else : ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    pristine.addValidator(fileInput, function() {
        ...
        return true; // Tidak upload file baru = valid
